...
O3 Code is moving toward npm/NPX-owned JavaScript, UI assets, router code, and patch metadata. Checked-in native runtime payloads make the repo and any future npm package large, platform-specific, and responsible for redistributing compiled Codex App material.
...
O3 Code does not keep host-native compiled runtime binaries in the repo. The default Native Resource Provider is the user's installed official Codex Desktop app:
...
The repo no longer contains the checked-in Codex CLI, app-server-runtime binaries, native add-ons, helper apps, or native plugin prebuilds.
...
Source Refresh must skip host-native compiled payloads. The refresh copies source, metadata, and acceptable assets, then relies on the installed app for native runtime material.
