import { Cn as e } from "./src-DAzAmbVS.js";
import { is as t, zt as n } from "./app-server-manager-signals-Csopz8aM.js";
import {
  C as r,
  L as i,
  _t as a,
...
import { t as c } from "./invalidate-queries-and-broadcast-CcloHaCj.js";
import { r as l } from "./apps-queries-BoCPY2Ov.js";
import { t as u } from "./codex-api-error-ClgzyL9y.js";
async function d({ hostId: t, queryClient: n, refreshAppsList: r }) {
  let i = n.getQueryData([`apps`, `list`, t]),
    a = await r();
  return f({ previousApps: i, nextApps: a })
    ? (await o(`ambient-suggestions-refresh`, {
        params: {
          hostId: t,
          projectRoot: e(`~`),
      ...
      await Promise.all([
        n.invalidateQueries({ queryKey: s(`ambient-suggestions`) }),
        n.invalidateQueries({ queryKey: s(`ambient-suggestions-refresh`) }),
      ]),
      a)
    : a;
}
...
function g() {
  let e = i(),
    t = c(),
    { clearPendingAppConnect: a, getPendingAppConnectForCallbackUrl: o } = h();
  return async ({ fullRedirectUrl: i }) => {
    let s = i.trim(),
      c = s.length === 0 ? null : o(s),
      f = c?.hostId ?? `local`,
      p = c?.oauthState ?? v(s) ?? void 0;
    if (s.length === 0)
      return (a({ oauthState: p }), { kind: `missing-callback-data` });
    try {
      return {
        kind: `success`,
        appName:
          (
            await n.safePost(`/aip/connectors/links/oauth/callback`, {
              requestBody: { full_redirect_url: s },
            })
          ).link?.name?.trim() ||
          c?.appName ||
          `App`,
      ...
      r.error(`Failed to finish app OAuth callback`, {
        safe: {},
        sensitive: { error: e },
      });
...
//# sourceMappingURL=app-connect-oauth-D-r_3-C5.js.map
