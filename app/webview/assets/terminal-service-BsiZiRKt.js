import { qi as e } from "./app-server-manager-signals-Csopz8aM.js";
import { y as t } from "./setting-storage-EK1Te68s.js";
var n = 16e3,
  r = new RegExp(String.raw`[\u0000-\u001F\u007F]`, `g`),
  i = [`: `, ` — `, ` - `, ` | `],
  a = class {
    listeners = new Map();
    ...
    constructor() {
      (t.subscribe(`terminal-data`, (e) => {
        ...
        t.subscribe(`close-terminal-session`, (e) => {
          this.closeForConversation(e.conversationId);
        }),
        t.subscribe(`clear-active-terminal`, () => {
          for (let e of this.listeners.values()) e.onClearActive?.();
        }));
    }
    create(e) {
      let n = e.sessionId ?? this.makeId();
      return (
        e.conversationId && this.setSessionMapping(n, e.conversationId),
        this.attachedSessionIds.delete(n),
        t.dispatchMessage(`terminal-create`, { ...e, sessionId: n }),
        n
      );
    }
    attach(e) {
      (e.conversationId &&
        ...
        t.dispatchMessage(`terminal-attach`, e));
    }
    write(e, t) {
    ...
    setTitle(e, t) {
      let n = s(t, this.getOrCreateSnapshot(e).cwd);
      this.updateSnapshotMetadata(e, { title: n, rawShellTitle: t });
    }
    resize(e, n, r) {
      let i = this.resizeSizeBySessionId.get(e);
      (i?.cols === n && i.rows === r) ||
        (this.resizeSizeBySessionId.set(e, { cols: n, rows: r }),
        t.dispatchMessage(`terminal-resize`, {
          sessionId: e,
          cols: n,
          rows: r,
        }));
    ...
    close(e) {
      (this.deleteSessionMapping(e),
        t.dispatchMessage(`terminal-close`, { sessionId: e }));
    }
    closeForConversation(e) {
      let n = String(e),
        r = this.conversationSessions.get(n);
      if (r != null) {
        for (let e of r.sessionIds)
          (this.deleteSessionMapping(e, {
        ...
        (this.conversationSessions.delete(n),
          this.notifyConversationListeners(n));
      }
    }
    ...
    appendSnapshotBuffer(e, t) {
      let r = this.getOrCreateSnapshot(e),
        i = `${r.buffer}${t}`;
      this.sessionSnapshots.set(e, {
        ...r,
        buffer: i.slice(-n),
        truncated: i.length > n,
      });
    }
    ...
    updateSnapshotMetadata(e, t) {
      ...
      if (
        ...
        n.shell === o.shell &&
        n.title === o.title &&
        n.rawShellTitle === o.rawShellTitle
      )
        return;
      this.sessionSnapshots.set(e, o);
      let c = this.sessionConversations.get(e);
      if (c == null) return;
      let l = this.conversationSessions.get(c);
      l != null &&
        (this.conversationSessions.set(c, {
          activeSessionId: l.activeSessionId,
          sessionIds: l.sessionIds,
          ...this.getConversationSessionMetadata(l.sessionIds),
        }),
        this.notifyConversationListeners(c));
    }
    ...
    sendOrQueueSessionAction(e, t) {
      if (this.canSendSessionAction(e)) {
    ...
    sendPendingSessionActions(e) {
      if (!this.canSendSessionAction(e)) return;
      let t = this.pendingSessionActions.get(e);
      if (t != null) {
        this.pendingSessionActions.delete(e);
        for (let n of t) this.sendSessionAction(e, n);
      }
    }
    canSendSessionAction(e) {
      return this.attachedSessionIds.has(e) && this.listeners.has(e);
    }
    sendSessionAction(e, n) {
      switch (n.type) {
        case `terminal-run-action`:
          t.dispatchMessage(`terminal-run-action`, {
            sessionId: e,
            cwd: n.cwd,
            command: n.command,
          });
          return;
        case `terminal-write`:
          t.dispatchMessage(`terminal-write`, { sessionId: e, data: n.data });
          return;
      }
    }
  };
function o(e) {
  return e.replace(r, ``).trim();
}
function s(t, n) {
  let r = o(t);
  if (r.length === 0) return null;
  let i = o(n);
  return i.length === 0 ? r : r === i || r === e(i) ? null : c(r, i);
}
function c(t, n) {
  let r = e(n);
  for (let e of i) {
    let i = `${n}${e}`;
    if (t.startsWith(i)) return t.slice(i.length).trim();
    let a = `${r}${e}`;
...
var l = new a();
export { l as t };
//# sourceMappingURL=terminal-service-BsiZiRKt.js.map
