...
function _(e, t) {
  let n = [],
    r = e.cwd == null ? t : W(e.cwd),
    {
      ...
      personalityChangedItems: h,
      forkedFromConversationItems: g,
      modelChangedItems: _,
      modelReroutedItems: y,
      todoListItem: b,
      proposedPlanItem: S,
      planImplementationItem: C,
      mcpServerElicitationItems: w,
  ...
  for (let e of f) F(O, v(e, r));
  (b != null && F(O, v(b, r)),
    S != null && F(O, v(S, r)),
    C != null && F(O, v(C, r)));
  for (let e of w) F(O, v(e, r));
  for (let e of T) F(O, v(e, r));
  (E != null && F(O, v(E, r)),
    D != null && F(O, v(D, r)),
    O.length > 0 &&
      n.push(
...
`),
    ...
    a != null && F(n, x(a, { pathContext: r })));
  for (let e of m) F(n, v(e, r));
  for (let e of h) F(n, v(e, r));
  for (let e of g) F(n, v(e, r));
  return n.length === 0
    ? null
    : n.join(`
...
function v(e, t) {
  switch (e.type) {
    ...
    case `turn-diff`:
      return w(e);
    case `web-search`:
      return `Searched web for ${q(e.query)}`;
    case `generated-image`:
      return k(e.src, e.status);
    case `userInput`:
      return P(
...
function y(e, { escapeDetailsTags: t = !1, pathContext: n = d } = {}) {
  let r = [],
...
`,
  ...
  return (
    ...
    e.pullRequestCheckCount != null &&
      e.pullRequestCheckCount > 0 &&
      n.push(`Pull request checks: ${e.pullRequestCheckCount}`),
    n
  );
}
function x(e, { escapeDetailsTags: t = !1, pathContext: n = d } = {}) {
...
function w(e) {
  return I(`Diff`, Y(`diff`, e.unifiedDiff));
}
function T(e) {
  let t = [];
  return (
...
function E(e) {
  ...
  if (
    ...
    t.push(Y(`json`, Z(e.invocation.arguments))),
    e.result == null)
  )
    return (
...
`);
  return (
    ...
    e.result.structuredContent != null &&
      t.push(Y(`json`, Z(e.result.structuredContent))),
    t.join(`
  ...
  switch (e.type) {
    ...
    case `embedded_resource`: {
      ...
      return e.resource.text != null && e.resource.text.trim().length > 0
        ? `${G(`Resource: ${t}`)}\n\n${Y(`text`, e.resource.text)}`
        : G(`Resource: ${t}`);
    }
    case `unknown`:
      return Y(`json`, Z(e.raw));
  }
}
function O(e) {
...
`),
    ...
    return I(
      n,
      e.units.map((e) => A(e, t)).filter((e) => e != null).join(`
...
function Te(e) {
  ...
  (Q(t, e.createdFileCount, `Created`, `created`, `file`, `files`),
    ...
    Q(t, e.deletedFileCount, `Deleted`, `deleted`, `file`, `files`));
  let n = [
    $(e.exploredFileCount, `file`, `files`),
    $(e.searchCount, `search`, `searches`),
    $(e.listCount, `list`, `lists`),
  ].filter((e) => e != null);
  return (
    n.length > 0 &&
      t.push(`${t.length === 0 ? `Explored` : `explored`} ${n.join(`, `)}`),
    Q(t, e.approvedRequestCount, `Approved`, `approved`, `request`, `requests`),
    Q(t, e.deniedRequestCount, `Denied`, `denied`, `request`, `requests`),
    Q(t, e.commandCount, `Ran`, `ran`, `command`, `commands`),
    Q(t, e.mcpToolCallCount, `Called`, `called`, `tool`, `tools`),
    Q(t, e.webSearchCount, `Searched web`, `searched web`, `time`, `times`),
...
export { g as renderConversationMarkdown };
//# sourceMappingURL=conversation-markdown-C9YbuaJQ.js.map
