...
import { t as u } from "./invalidate-queries-and-broadcast-CPrIeBVQ.js";
import { S as d, s as f } from "./config-queries-2cSUtzNR.js";
import { r as p } from "./toast-signal-mlbIRbst.js";
import { t as m } from "./apps-queries-Bm5YDQ9P.js";
var h = l(),
  g = `apps`,
  _ = t({
    enableSuccess: {
...
function v(t) {
  ...
  l[0] === S
    ? (w = l[1])
    : ((w = async () => {
        await Promise.all([S(m), S(f), S([`user-saved-config`])]);
      }),
      (l[0] = S),
      (l[1] = w));
  let T = w,
    E;
  l[2] !== g ||
  l[3] !== b?.configWriteTarget?.expectedVersion ||
  l[4] !== b?.configWriteTarget?.filePath
    ? ((E = async (t) => {
  ...
  l[6] === x
    ...
    : ((D = async (e) => {
        ...
        await x.cancelQueries({ queryKey: m });
        let r = x.getQueryData(m);
        return (
          r &&
            x.setQueryData(
              m,
  ...
  l[8] !== C || l[9] !== x || l[10] !== v
    ? ((O = (e, t) => {
        let { appId: n, enabled: r } = t,
          i = x.getQueryData(m)?.find((e) => e.id === n)?.name ?? n;
        v.get(p).success(
      ...
      (k = (e, t, r) => {
        (n.error(`Failed to update app enablement`, {
          ...
          v.get(p).danger(C.formatMessage(_.updateError)),
          r?.previousApps && x.setQueryData(m, r.previousApps));
      }),
      (l[8] = C),
      (l[9] = x),
      (l[10] = v),
    ...
    : ((O = l[11]), (k = l[12]));
  let A;
  l[13] !== T || l[14] !== E || l[15] !== D || l[16] !== O || l[17] !== k
    ? ((A = {
        mutationFn: E,
        onMutate: D,
        onSuccess: O,
    ...
    : (A = l[18]);
  let j = c(A),
    M = j.isPending ? (j.variables?.appId ?? null) : null,
    N;
  return (
    l[19] !== j.isPending || l[20] !== j.mutateAsync || l[21] !== M
      ? ((N = {
          setAppEnabled: j.mutateAsync,
          isUpdating: j.isPending,
          updatingAppId: M,
        }),
        (l[19] = j.isPending),
        (l[20] = j.mutateAsync),
...
export { v as t };
//# sourceMappingURL=apps-availability-DuFJh-dy.js.map
