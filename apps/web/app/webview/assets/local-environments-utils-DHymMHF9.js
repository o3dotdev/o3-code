...
import { xt as r, z as i } from "./setting-storage-kJblH-wH.js";
import { t as a } from "./bug-CW5XFoH5.js";
import { t as o } from "./settings.cog-CViPO9-c.js";
...
var p = [
  ...
  m = [`darwin`, `linux`, `win32`],
  h = `cd "$CODEX_WORKTREE_PATH"
pip install -r requirements.txt
npm install
./run/setup.sh`,
  g = `npm run dev`,
  _ = `docker compose down --remove-orphans
rm -rf .cache/tmp`;
function v(e) {
  return { id: C(), name: e, icon: `tool`, command: ``, platform: null };
}
function y(e) {
  return e.length > 0
    ? e.map((e) => ({ ...e, id: C(), platform: e.platform ?? null }))
    : [];
}
function b(e) {
  return {
...
function x({
  ...
  for (let e of s)
    (c.push(`[[actions]]`),
      c.push(`name = ${E(e.name)}`),
      e.icon && c.push(`icon = ${E(e.icon)}`),
      c.push(`command = ${E(e.command)}`),
      e.platform && c.push(`platform = ${E(e.platform)}`),
      c.push(``));
  return `${c
    .join(
      `
...
function w(e, t, n, r) {
  ...
  if (
    ((t === `setup` || n.length > 0 || i.length > 0) &&
      (e.push(``), e.push(`[${t}]`), e.push(`script = ${E(n)}`)),
    i.length !== 0)
  ) {
    e.push(``);
    for (let [n, r] of i.entries())
      (e.push(`[${t}.${r.platform}]`),
        e.push(`script = ${E(r.script)}`),
        n < i.length - 1 && e.push(``));
  }
}
function T(e) {
...
//# sourceMappingURL=local-environments-utils-DHymMHF9.js.map
