  ...
  let d = o.concat({ begin: /\{/, end: /\}/, contains: a }),
    ...
    g = {
      ...
      contains: [
        ...
        c(`keyword`, `all\\b`),
        c(`variable`, `@\\{` + r + `\\}`),
        { begin: `\\b(` + oe.join(`|`) + `)\\b`, className: `selector-tag` },
        t.CSS_NUMBER_MODE,
        c(`selector-tag`, i, 0),
        c(`selector-id`, `#` + i),
...
function G(...e) {
  return `(` + (Ve(e).capture ? `` : `?:`) + e.map((e) => H(e)).join(`|`) + `)`;
}
var K = (e) => W(/\b/, e, /\w$/.test(e) ? /\b/ : /\B/),
  He = [`Protocol`, `Type`].map(K),
  Ue = [`init`, `self`].map(K),
  We = [`Any`, `Self`],
  q = [
    `actor`,
...
function et(e) {
  let t = { match: /\s+/, relevance: 0 },
    ...
    i = { match: [/\./, G(...He, ...Ue)], className: { 2: `keyword` } },
    ...
    c = { $pattern: G(/\b\w+/, /#\w+/), keyword: o.concat(qe), literal: Ge },
    l = [i, a, s],
    u = [
      { match: W(/\./, G(...Je)), relevance: 0 },
      { className: `built_in`, match: W(/\b/, G(...Je), /(?=\()/) },
    ],
    d = { match: /->/, relevance: 0 },
    f = [
      d,
    ...
    O = [
      {
        ...
        starts: {
          contains: [
            { begin: /\(/, end: /\)/, keywords: $e, contains: [...f, h, x] },
          ],
        },
      },
      { scope: `keyword`, match: W(/@/, G(...Qe), U(G(/\(/, /\s+/))) },
