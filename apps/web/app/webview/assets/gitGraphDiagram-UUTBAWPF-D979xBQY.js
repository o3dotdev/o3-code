...
var te = t(function (e) {
  ...
  ie = t(function (e) {
    ...
    let o = S();
    ((r = f.sanitizeText(r, o)),
      (t = f.sanitizeText(t, o)),
      (a = a?.map((e) => f.sanitizeText(e, o))));
    let s = {
    ...
    ((C.records.head = s),
      n.info(`main branch`, o.mainBranchName),
      C.records.commits.has(s.id) && n.warn(`Commit ID ${s.id} already exists`),
      C.records.commits.set(s.id, s),
      C.records.branches.set(C.records.currBranch, s.id),
      n.debug(`in pushCommit ` + s.id));
  }, `commit`),
  ae = t(function (e) {
    let t = e.name,
      r = e.order;
    if (((t = f.sanitizeText(t, S())), C.records.branches.has(t)))
      throw Error(
    ...
    (C.records.branches.set(
      ...
      C.records.branchConfig.set(t, { name: t, order: r }),
      E(t),
      n.debug(`in createBranch`));
  }, `branch`),
  oe = t((e) => {
    let t = e.branch,
      r = e.id,
      ...
      a = e.tags,
      o = S();
    ((t = f.sanitizeText(t, o)), (r &&= f.sanitizeText(r, o)));
    let s = C.records.branches.get(C.records.currBranch),
      c = C.records.branches.get(t),
    ...
    ((C.records.head = p),
      C.records.commits.set(p.id, p),
      C.records.branches.set(C.records.currBranch, p.id),
      n.debug(C.records.branches),
      n.debug(`in mergeBranch`));
  }, `merge`),
  se = t(function (e) {
    let t = e.id,
      r = e.targetId,
    ...
    if (
      ((t = f.sanitizeText(t, o)),
      (r = f.sanitizeText(r, o)),
      (i = i?.map((e) => f.sanitizeText(e, o))),
      (a = f.sanitizeText(a, o)),
      !t || !C.records.commits.has(t))
    ) {
      let e = Error(
    ...
    if (!r || !C.records.commits.has(r)) {
      ...
      ((C.records.head = l),
        ...
        n.debug(C.records.branches),
        n.debug(`in cherryPick`));
    }
  }, `cherryPick`),
  E = t(function (e) {
    if (((e = f.sanitizeText(e, S())), C.records.branches.has(e))) {
...
function O(e) {
  ...
  for (let e in C.records.branches)
    C.records.branches.get(e) === t.id && i.push(e);
  if (
    (n.debug(i.join(` `)),
...
var ce = t(function () {
  ...
  Ie = t((e, t, n, r) => {
    ...
    if (c) {
      ...
      K === `TB`
        ? o.x < s.x
          ? (h = `M ${o.x} ${o.y} L ${n - f} ${o.y} ${d} ${n} ${o.y + p} L ${n} ${s.y - f} ${u} ${n + p} ${s.y} L ${s.x} ${s.y}`)
          : ((m = B.get(t.branch)?.index),
            (h = `M ${o.x} ${o.y} L ${n + f} ${o.y} ${u} ${n} ${o.y + p} L ${n} ${s.y - f} ${d} ${n - p} ${s.y} L ${s.x} ${s.y}`))
        : K === `BT`
          ? o.x < s.x
            ? (h = `M ${o.x} ${o.y} L ${n - f} ${o.y} ${u} ${n} ${o.y - p} L ${n} ${s.y + f} ${d} ${n + p} ${s.y} L ${s.x} ${s.y}`)
            : ((m = B.get(t.branch)?.index),
              (h = `M ${o.x} ${o.y} L ${n + f} ${o.y} ${d} ${n} ${o.y - p} L ${n} ${s.y + f} ${u} ${n - p} ${s.y} L ${s.x} ${s.y}`))
          : o.y < s.y
            ? (h = `M ${o.x} ${o.y} L ${o.x} ${e - f} ${u} ${o.x + p} ${e} L ${s.x - f} ${e} ${d} ${s.x} ${e + p} L ${s.x} ${s.y}`)
            : ((m = B.get(t.branch)?.index),
              (h = `M ${o.x} ${o.y} L ${o.x} ${e + f} ${d} ${o.x + p} ${e} L ${s.x - f} ${e} ${u} ${s.x} ${e - p} L ${s.x} ${s.y}`));
    } else
      ((u = `A 20 20, 0, 0, 0,`),
        (d = `A 20 20, 0, 0, 1,`),
        (f = 20),
          ...
          ? (o.x < s.x &&
              (h =
                n.type === x.MERGE && t.id !== n.parents[0]
                  ? `M ${o.x} ${o.y} L ${o.x} ${s.y - f} ${u} ${o.x + p} ${s.y} L ${s.x} ${s.y}`
                  : `M ${o.x} ${o.y} L ${s.x - f} ${o.y} ${d} ${s.x} ${o.y + p} L ${s.x} ${s.y}`),
            o.x > s.x &&
              ((u = `A 20 20, 0, 0, 0,`),
              (d = `A 20 20, 0, 0, 1,`),
              (f = 20),
              ...
              (h =
                n.type === x.MERGE && t.id !== n.parents[0]
                  ? `M ${o.x} ${o.y} L ${o.x} ${s.y - f} ${d} ${o.x - p} ${s.y} L ${s.x} ${s.y}`
                  : `M ${o.x} ${o.y} L ${s.x + f} ${o.y} ${u} ${s.x} ${o.y + p} L ${s.x} ${s.y}`)),
  ...
  Re = t((e, t, n, r) => {
    let { look: i, theme: a, themeVariables: o } = l(),
    ...
    t.forEach((e, t) => {
      ...
      let l = K === `TB` || K === `BT` ? o : u ? o + L / 2 + 1 : o - 2,
        p = f.append(`line`);
      (p.attr(`x1`, 0),
        p.attr(`y1`, l),
        p.attr(`x2`, G),
      ...
      (i === `neo` && g.attr(`data-look`, `neo`),
        g
          ...
          .attr(
            `style`,
            i === `neo` ? `filter:${u ? `url(#${r}-drop-shadow)` : s}` : ``,
          )
          .attr(`rx`, y)
          .attr(`ry`, y)
        ...
        K === `TB`
          ...
          : K === `BT`
            ...
            : g.attr(`transform`, `translate(-19, ` + (l - 12 - x / 2) + `)`));
    });
  }, `drawBranches`),
...
`,
      ...
      K = o.getDirection();
      let f = e(`[id="${r}"]`),
        { look: p, theme: m, themeVariables: g } = l(),
        {
          useGradient: _,
          gradientStart: v,
          gradientStop: y,
      ...
      (u.forEach((e, t) => {
        ...
        let o = n.getBBox();
        ((x = ze(e.name, x, t, o, c)), a.remove(), i.remove(), r.remove());
      }),
        Y(f, U, !1, s),
        s.showBranches && Re(f, u, s, r),
        Le(f, U),
        Y(f, U, !0, s),
        h.insertTitle(
          f,
        ...
        d(void 0, f, s.diagramPadding, s.useMaxWidth));
    }, `draw`),
  },
  Z = 8,
  ...
  Ge = t((e) => {
    let { theme: t, themeVariables: n } = m(),
...
//# sourceMappingURL=gitGraphDiagram-UUTBAWPF-D979xBQY.js.map
