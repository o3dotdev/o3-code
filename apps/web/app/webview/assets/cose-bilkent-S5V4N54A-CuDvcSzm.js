...
import { t as r } from "./cytoscape-cose-bilkent.js";
  ...
  return new Promise((r) => {
    ...
    (i.remove(),
      ...
      o
        ...
        .run(),
      o.ready((e) => {
        (a.info(`Cytoscape ready`, e), r(o));
      }));
  });
}
i(l, `createCytoscapeInstance`);
function u(e) {
...
i(d, `extractPositionedEdges`);
async function f(e, t) {
  a.debug(`Starting cose-bilkent layout algorithm`);
  try {
    p(e);
    let t = await l(e),
      n = u(t),
      r = d(t);
    return (
      a.debug(`Layout completed: ${n.length} nodes, ${r.length} edges`),
      { nodes: n, edges: r }
    );
  } catch (e) {
    throw (a.error(`Error in cose-bilkent layout algorithm:`, e), e);
  }
}
i(f, `executeCoseBilkentLayout`);
function p(e) {
...
var m = i(
  async (
    ...
    let m = p.insert(`g`).attr(`class`, `subgraphs`),
      h = p.insert(`g`).attr(`class`, `edgePaths`),
      g = p.insert(`g`).attr(`class`, `edgeLabels`),
      _ = p.insert(`g`).attr(`class`, `nodes`);
    (s.debug(`Inserting nodes into DOM for dimension calculation`),
      await Promise.all(
        e.nodes.map(async (t) => {
          if (t.isGroup) {
            let e = { ...t };
            ((d[t.id] = e), (u[t.id] = e), await n(m, t));
          } else {
            let n = { ...t };
            u[t.id] = n;
            let r = await o(_, t, {
...
export { m as render };
//# sourceMappingURL=cose-bilkent-S5V4N54A-CuDvcSzm.js.map
